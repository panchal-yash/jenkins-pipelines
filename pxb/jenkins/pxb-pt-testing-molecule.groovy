    ...
    library changelog: false, identifier: "lib@pxb-ms-ps-package-testing-pxb-80-pt", retriever: modernSCM([
        $class: 'GitSCMSource',
    ...
    parameters {
        choice(
            choices: ['pxb81', 'pxb_80', 'pxb24', 'pxb_innovation_lts'],
    ...
    options {
        ...
        stages {
            stage('Set Build Name'){
                steps {
                    script {
                        currentBuild.displayName = "${product_to_test}-${server_to_test}-${scenario_to_test}"
                    }
                }
            }
            ...
            stage('Checkout') {
                steps {
                    deleteDir()
                    git poll: false, branch: "pxb-ms-ps-package-testing-pxb-80-pt", url: "https://github.com/panchal-yash/package-testing.git"
                }
            }
            ...
            stage('RUN TESTS') {
                        steps {
                            script {
                                ...
                                withEnv(envMap) {
                                    moleculeParallelTest(pxbPackageTesting(), "molecule/pxb-package-testing/")
                                }
