...
*/
    ...
    post {
        success {
            ...
            slackNotify("#dev-server-qa", "#00FF00", "[${JOB_NAME}]: Triggering Builds for Package Testing for ${PXB_REPO} ${BRANCH} - [${BUILD_URL}]")
            ...
            script {
                ...
                sh """
                    ...
                    if [[ ${PXB_REPO} == "pxb-80" ]]; then
                        ...
                        echo "Selected PXB-81"
                        OLD_PXB81_VER=\$(cat VERSIONS | grep PXB81_VER | cut -d '=' -f2- )
                        OLD_PXB81PKG_VER=\$(cat VERSIONS | grep PXB81PKG_VER | cut -d '=' -f2- )
                        sed -i s/PXB81_VER=\$OLD_PXB81_VER/PXB81_VER='"'${XB_VERSION_MAJOR}.${XB_VERSION_MINOR}.${XB_VERSION_PATCH}'"'/g VERSIONS
                        sed -i s/PXB81PKG_VER=\$OLD_PXB81PKG_VER/PXB81PKG_VER='"'${XB_VERSION_EXTRA}'"'/g VERSIONS
                    else 
                ...
                if ( "${PXB_REPO}" == "pxb-80" ) {
                    build job: 'pxb-package-testing-all-8.1-test', propagate: false, wait: false, parameters: [string(name: 'product_to_test', value: 'pxb80'),string(name: 'install_repo', value: "testing"),string(name: 'git_repo', value: "https://github.com/Percona-QA/package-testing.git")]
                } else if ( "${PXB_REPO}" == "pxb-81" ) {
                    build job: 'pxb-package-testing-all-8.1-test', propagate: false, wait: false, parameters: [string(name: 'product_to_test', value: 'pxb81'),string(name: 'install_repo', value: "testing"),string(name: 'git_repo', value: "https://github.com/Percona-QA/package-testing.git")]
                } else {
                    echo "Wrong mention in PXB_REPO"
                }
