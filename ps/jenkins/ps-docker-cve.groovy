...
pipeline {
            ...
            steps {
                sh """
                    ...
                    /usr/local/bin/trivy -q image --format template --template @junit.tpl  -o trivy-hight-junit.xml \
                                --timeout 10m0s --ignore-unfixed --exit-code 0 --severity HIGH,CRITICAL "${params.DOCKER_REPO}/percona-server:${params.PS_VERSION}"
                """
            }
            post {
                always {
    ...
    post {
        ...
        success {
            slackNotify("#dev-server-qa", "#00FF00", "[${JOB_NAME}]: Testing PS docker images for CVE - succeed")
        }
        unstable {
            slackNotify("#dev-server-qa", "#F6F930", "[${JOB_NAME}]: Testing PS docker images for CVE - some issues found: [${BUILD_URL}testReport/]")
        }
        failure {
            slackNotify("#dev-server-qa", "#FF0000", "[${JOB_NAME}]: Testing PS docker images for CVE - unexpected failure: [${BUILD_URL}]")
        }
    }
}
