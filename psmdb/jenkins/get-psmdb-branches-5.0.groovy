        ...
        stage('Get release branches') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: '24e68886-c552-4033-8503-ed85bbaa31f3', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_50.properties || EC=\$?
			...
			if [ \${EC} = 1 ]; then
			  ...
			  echo "BRANCH_NAME=\${BRANCH_NAME}" > branch_commit_id_50.properties
			  echo "COMMIT_ID=\${COMMIT_ID}" >> branch_commit_id_50.properties
			  echo "MONGO_TOOLS_TAG=\${MONGO_TOOLS_TAG}" >> branch_commit_id_50.properties

			  aws s3 cp branch_commit_id_50.properties s3://percona-jenkins-artifactory/percona-server-mongodb/
                          echo "START_NEW_BUILD=NO" > startBuild
			else
                          aws s3 cp s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_50.properties .
			  source branch_commit_id_50.properties
			  ...
			  echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_50.properties
			  echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_50.properties
			  echo "MONGO_TOOLS_TAG=\${MONGO_TOOLS_TAG}" >> branch_commit_id_50.properties
			  aws s3 cp branch_commit_id_50.properties s3://percona-jenkins-artifactory/percona-server-mongodb/
                        fi
                    """
                ...
                script {
                    ...
                    BRANCH_NAME = sh(returnStdout: true, script: "source branch_commit_id_50.properties; echo \${BRANCH_NAME}").trim()
                    COMMIT_ID = sh(returnStdout: true, script: "source branch_commit_id_50.properties; echo \${COMMIT_ID}").trim()
                    VERSION = sh(returnStdout: true, script: "source branch_commit_id_50.properties; echo \${BRANCH_NAME} | cut -d - -f 2 ").trim()
                    RELEASE = sh(returnStdout: true, script: "source branch_commit_id_50.properties; echo \${BRANCH_NAME} | cut -d - -f 3 ").trim()
                    MONGO_TOOLS_TAG = sh(returnStdout: true, script: "source branch_commit_id_50.properties; echo \${MONGO_TOOLS_TAG}").trim()
                }
