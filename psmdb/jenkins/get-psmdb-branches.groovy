        ...
        stage('Get release branches') {
            steps {
                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: '24e68886-c552-4033-8503-ed85bbaa31f3', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY']]) {
                    sh """
                        EC=0
                        aws s3 ls s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_36.properties || EC=\$?

			if [ \${EC} = 1 ]; then
			  LATEST_RELEASE_BRANCH=\$(git -c 'versionsort.suffix=-' ls-remote --heads --sort='v:refname' ${GIT_REPO} release-3.6\\* | tail -1)
			  BRANCH_NAME=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d "/" -f 3)
			  COMMIT_ID=\$(echo \${LATEST_RELEASE_BRANCH} | cut -d " " -f 1)

			  echo "BRANCH_NAME=\${BRANCH_NAME}" > branch_commit_id_36.properties
			  echo "COMMIT_ID=\${COMMIT_ID}" >> branch_commit_id_36.properties

			  aws s3 cp branch_commit_id_36.properties s3://percona-jenkins-artifactory/percona-server-mongodb/
                          echo "START_NEW_BUILD=NO" > startBuild
			else
                          aws s3 cp s3://percona-jenkins-artifactory/percona-server-mongodb/branch_commit_id_36.properties .
			  source branch_commit_id_36.properties
			  ...
			  echo "BRANCH_NAME=\${LATEST_BRANCH_NAME}" > branch_commit_id_36.properties
			  echo "COMMIT_ID=\${LATEST_COMMIT_ID}" >> branch_commit_id_36.properties
                          aws s3 cp branch_commit_id_36.properties s3://percona-jenkins-artifactory/percona-server-mongodb/
                        fi
                    """
                }
